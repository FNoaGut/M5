# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 17:20:04 2020
...
sns.set()
#%% Funcion para representar dataframes por defecto como head
import IPython
...
#%% Analiza si estamos en el os de kaggle
def on_kaggle():
...
#%%
def read_data():
    INPUT_DIR = "/kaggle/input" if on_kaggle() else "C:/Users/laguila/Google Drive/ARC_KAGGLE/m5-datos"
    INPUT_DIR = f"{INPUT_DIR}"

    print("Reading files...")

    calendar = pd.read_csv(f"{INPUT_DIR}/calendar.csv").pipe(reduce_mem_usage)  # pasarlo a otra funcion con pipe
...
def encode_categorical(df, cols):
    for col in cols:
        # Leave NaN as it is.
        le = LabelEncoder()
        not_null = df[col][df[col].notnull()] #.isnull() es lo opuesto
